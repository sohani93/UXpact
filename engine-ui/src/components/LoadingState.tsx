...
function NodeMap() {
  ...
  useEffect(() => {
    ...
    if (!nodesRef.current) {
      ...
      for (let i = 1; i <= 5; i++) {
        connections.push([i, 6 + (i % 8)]);
        connections.push([i, 6 + ((i + 1) % 8)]);
      }
      for (let i = 6; i <= 13; i++) {
        connections.push([i, 14 + ((i - 6) % 10)]);
        connections.push([i, 14 + ((i - 5) % 10)]);
      }
    ...
    function draw() {
      ...
      connections.forEach(([a, b]: [number, number]) => {
        ...
        ctx.strokeStyle = activated
          ? isV
            ? `rgba(91,97,244,${0.18 + Math.sin(t * 2 + a) * 0.08})`
            : `rgba(20,213,113,${0.18 + Math.sin(t * 2 + a) * 0.08})`
          : isV
            ? "rgba(91,97,244,0.05)"
            : "rgba(20,140,89,0.05)";
        ctx.lineWidth = activated ? 1 : 0.5;
        ctx.stroke();
        if (activated) {
          ...
          ctx.fillStyle = `rgba(20,213,113,${0.5 * (1 - Math.abs(pulseT - 0.5) * 2)})`;
          ctx.fill();
        }
      });
      nodes.forEach((n: any) => {
      ...
      <div style={{ maxWidth: 1060, margin: "0 auto", padding: "8px 28px 60px", position: "relative", zIndex: 10 }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontFamily: "'Unbounded', sans-serif", fontSize: 26, fontWeight: 700, color: "#0B1C48", letterSpacing: "-0.5px", margin: "0 0 6px" }}>
            Scanning{" "}
            <span style={{ background: "linear-gradient(90deg, #186132, #14D571)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{domain}</span>
          </h1>
