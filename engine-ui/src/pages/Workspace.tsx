// @ts-nocheck
// ─── WORKSPACE ───
// One continuous workspace per site: Diagnosis, Conversion Blueprint (Current +
// Restructured/Vision Pro), and UX Pulse all live as sections of this single
// scrollable page — not separate routes. No numeric score anywhere, no
// exceptions: the product is the AI's narrative diagnosis, not a rule-engine
// grade.
...
function Pill({ text, v }: { text: string; v: "green" | "violet" }) {
  const s = v === "green" ? { background: "#D1FAE5", color: C.navy } : { background: "#E0E7FF", color: C.navy };
  return <div style={{ ...s, padding: "4px 12px", borderRadius: 6, fontSize: 12, fontWeight: 600, fontFamily: "'Space Grotesk', sans-serif", whiteSpace: "nowrap" }}>{text}</div>;
}

// ── Pin — numbered, single accent colour. No severity ranking: the AI's
// journey breaks aren't scored against each other, each is a real break. ──
function Pin({ finding, active, onClick, addressed }) {
...
function FixDrawer({ finding, onClose, addressed, onToggleAddressed }) {
...
function PinRow({ zone, activeId, setActiveId, findings, isAddressed }) {
...
function SectionNav({ current, onJump }: { current: string; onJump: (id: string) => void }) {
  const items = [
  ...
  return (
    <div style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(249,249,249,0.85)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", borderBottom: `1px solid ${C.border}` }}>
      <div style={{ maxWidth: 1120, margin: "0 auto", padding: "0 28px", display: "flex", gap: 4 }}>
        {items.map((it) => (
          <button key={it.id} onClick={() => onJump(it.id)} style={{
            padding: "12px 16px", background: "none", border: "none", cursor: "pointer",
            fontFamily: "'Space Grotesk', sans-serif", fontSize: 12.5, fontWeight: current === it.id ? 700 : 500,
            color: current === it.id ? C.forest : C.muted,
            borderBottom: current === it.id ? `2px solid ${C.forest}` : "2px solid transparent",
...
function DiagnosisSection({ auditData, journeyBreaks }: { auditData: any; journeyBreaks: any[] }) {
  const domain = auditData?.domain || "yoursite.com";
  const sortedBreaks = [...journeyBreaks].sort((a, b) => JOURNEY_STAGE_ORDER.indexOf(a.journeyStage) - JOURNEY_STAGE_ORDER.indexOf(b.journeyStage));

  return (
    <div id="diagnosis" style={{ maxWidth: 1120, margin: "0 auto", padding: "28px 28px 8px" }}>
      ...
      <div style={{ ...glass, borderRadius: 16, padding: "28px 28px 24px", marginBottom: 16 }}>
        {auditData?.narrative_verdict ? (
          <div style={{ borderRadius: 12, padding: "24px 26px", background: "linear-gradient(135deg,#186132 0%,#148C59 60%,#14D571 100%)", marginBottom: sortedBreaks.length > 0 ? 20 : 0 }}>
            ...
            <p style={{ fontSize: 16, fontWeight: 700, color: "#fff", lineHeight: 1.55, margin: 0, letterSpacing: "-0.2px" }}>{auditData.narrative_verdict}</p>
          </div>
        ) : (
        ...
        {sortedBreaks.length > 0 && (
          <div style={{ marginTop: 4 }}>
            ...
            {auditData?.current_archetype && auditData?.target_archetype && (
              <div style={{ fontSize: 11.5, color: C.muted, marginBottom: 14 }}>
                Read through the lens of moving from how {domain} presents today (<span style={{ fontWeight: 700, color: C.navy }}>{auditData.current_archetype}</span>) toward how it should for this goal (<span style={{ fontWeight: 700, color: C.navy }}>{auditData.target_archetype}</span>).
              </div>
            )}
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {sortedBreaks.map((jb, i) => (
                <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                  <div style={{ width: 22, height: 22, borderRadius: "50%", background: "rgba(91,97,244,0.12)", color: C.violet, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10.5, fontWeight: 700, flexShrink: 0, marginTop: 1 }}>{i + 1}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                      ...
                      {jb.element && <span style={{ fontSize: 11.5, fontWeight: 600, color: C.navy }}>{jb.element}</span>}
                    </div>
                    <p style={{ fontSize: 13, color: "#374151", lineHeight: 1.6, margin: "0 0 3px" }}>{jb.whatsHappening}</p>
                    <p style={{ fontSize: 12, color: C.muted, lineHeight: 1.55, margin: 0 }}><span style={{ fontWeight: 600 }}>Should instead: </span>{jb.whatShouldHappen}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {auditData?.revenue_leak_estimate && (
        <div style={{ ...glass, borderRadius: 16, padding: "24px 28px", marginBottom: 16 }}>
          ...
          <p style={{ fontSize: 13, color: C.muted, margin: "0 0 10px" }}>Grounded in the specific breaks found above, not a generic estimate.</p>
          <p style={{ fontSize: 15, fontWeight: 700, color: C.forest, margin: 0 }}>{auditData.revenue_leak_estimate} <span style={{ fontWeight: 500, color: C.muted, fontSize: 13 }}>estimated at risk</span></p>
        </div>
      )}
    </div>
  );
...
export default function Workspace({ auditId }: { auditId: string }) {
  ...
  const refreshActiveVariants = async () => {
    ...
    const { data: activeRows } = await supabase.from("deployed_variants").select("id, traffic_weight").eq("audit_id", auditId).eq("is_active", true);
    const rows = activeRows ?? [];
    setIsLive(rows.length > 0);
    setActiveVariants(rows);
    if (rows.length === 0) { setVariantEventCounts({}); return; }
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const supabase = getSupabase();
        const [{ data: auditRows, error: auditErr }, { data: journeyRows }, { data: versionRows }, { data: driftRows }] = await Promise.all([
        ...
        if (!audit && cached) {
          try {
            ...
            audit = {
              id: c.auditId, domain: c.domain, industry: "saas", created_at: c.createdAt,
              narrative_verdict: c.narrativeVerdict ?? null, revenue_leak_estimate: c.revenueLeakEstimate ?? null,
              current_archetype: c.currentArchetype ?? null, target_archetype: c.targetArchetype ?? null,
              dom_data: c.domData ?? null, raw_html: null,
            };
        ...
        const mappedJourney = (journeyRows ?? []).map((j: any) => ({
          ...
          whatsHappening: j.current_archetype_signal, whatShouldHappen: j.what_should_happen,
          reason: j.reason, fix: j.fix, aiPrompt: j.ai_prompt,
        }));
  ...
  // Current-view pins are the AI's real journey breaks, placed by the real page
  // zone they describe — each pin already carries its own fix + AI prompt from
  // the diagnosis call, no separate findings lookup.
  const pinFindings = journeyBreaks.map((jb, i) => ({
    id: `jb-${i}`,
    number: i + 1,
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!genResponse.ok || genJson.error || !genJson.jobId) { setGenError(genJson.message || "Generation failed. Try again."); setGeneratedHtml(null); return; }
      const draftResult = await pollGenerationJob(genJson.jobId);
      if ("error" in draftResult) { setGenError(draftResult.error); setGeneratedHtml(null); return; }
      ...
      if (!checkResponse.ok || checkJson.error || !checkJson.jobId) { setGeneratedHtml(draftResult.html); setActiveVersionId(null); return; }
      const finalResult = await pollGenerationJob(checkJson.jobId);
      if ("error" in finalResult) { setGeneratedHtml(draftResult.html); setActiveVersionId(null); return; }
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      else await refreshActiveVariants();
    } catch (err) {
  ...
  const handleAddLiveTestVariant = async () => {
    ...
    try {
      ...
      const json = await response.json();
      if (!response.ok || json.error) setDeployError(json.message || "Couldn't add this as a live test variant. Try again.");
      else await refreshActiveVariants();
    } catch (err) {
      setDeployError(err instanceof Error ? err.message : "Couldn't reach the deploy service.");
    } finally {
  ...
  if (loadError && !auditData) {
    return <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", color: C.navy, fontFamily: "'Space Grotesk',sans-serif" }}>{loadError}</div>;
  }
  ...
  return (
    <div className="fade-in" style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Space Grotesk',sans-serif", position: "relative" }}>
      ...
      <SectionNav current={currentSection} onJump={jumpTo} />

      <DiagnosisSection auditData={auditData} journeyBreaks={journeyBreaks} />

      {/* ── Conversion Blueprint ─────────────────────────────────────── */}
      <div id="blueprint" style={{ maxWidth: 1120, margin: "0 auto", padding: "36px 28px 8px" }}>
        <h1 style={{ fontFamily: "'Unbounded',sans-serif", fontSize: 26, fontWeight: 700, color: C.navy, letterSpacing: "-0.5px", margin: "0 0 4px" }}>
      ...
      {facView === "current" ? (
        <div style={{ maxWidth: 1120, margin: "0 auto", padding: "0 28px 0", display: "flex", gap: 16, alignItems: "flex-start" }}>
          ...
          <div style={{ position: "sticky", top: 60, width: 340, flexShrink: 0 }}>
            {activeFinding ? (
              ...
              <div style={{ background: "rgba(255,255,255,0.4)", backdropFilter: "blur(24px)", borderRadius: 14, border: "1px solid rgba(220,38,38,0.2)", padding: "36px 24px", textAlign: "center" }}>
                <div style={{ fontFamily: "'Unbounded', sans-serif", fontSize: 13, fontWeight: 700, color: "#DC2626", marginBottom: 8 }}>Couldn't load this workspace</div>
                <div style={{ fontSize: 12, color: C.muted, lineHeight: 1.6 }}>{loadError} Try refreshing the page.</div>
              </div>
        ...
        <div style={{ maxWidth: 1120, margin: "0 auto", padding: "0 28px 40px", display: "grid", gridTemplateColumns: "320px 1fr", gap: 20, alignItems: "flex-start" }}>
          <div style={{ background: "rgba(255,255,255,0.5)", backdropFilter: "blur(20px)", borderRadius: 14, border: "1px solid rgba(255,255,255,0.7)", boxShadow: "0 8px 32px rgba(0,0,0,0.05)", padding: "18px 18px 20px", position: "sticky", top: 60 }}>
            {journeyBreaks.length > 0 && (
              <>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>
                  {[...journeyBreaks].sort((a, b) => JOURNEY_STAGE_ORDER.indexOf(a.journeyStage) - JOURNEY_STAGE_ORDER.indexOf(b.journeyStage)).map((jb, i) => (
            ...
            <div style={{ borderRadius: 14, overflow: "hidden", background: "#fff", border: "1px solid rgba(255,255,255,0.7)", boxShadow: "0 8px 32px rgba(0,0,0,0.06)", minHeight: 520, display: "flex", flexDirection: "column" }}>
              <div style={{ background: "rgba(0,0,0,0.03)", borderBottom: `1px solid ${C.border}`, padding: "8px 16px", display: "flex", alignItems: "center", gap: 8 }}>
                ...
                {generatedHtml && !generating && (
                  <div style={{ display: "flex", gap: 6 }}>
                    ...
                    {isLive ? (
                      <>
                        <button onClick={handleAddLiveTestVariant} disabled={addingLiveTest} style={{ padding: "5px 12px", borderRadius: 7, border: "none", cursor: addingLiveTest ? "default" : "pointer", background: addingLiveTest ? "rgba(91,97,244,0.3)" : "rgba(91,97,244,0.12)", color: C.violet, fontSize: 11, fontWeight: 700 }}>{addingLiveTest ? "Adding…" : "Add as live test variant"}</button>
              ...
              {isLive && (
                <div style={{ padding: "10px 16px", background: "rgba(20,140,89,0.06)", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: C.emerald }}>Live on {realDomain} —</span>
                  <code style={{ fontSize: 10.5, color: "#374151", background: "rgba(255,255,255,0.7)", padding: "3px 8px", borderRadius: 5, fontFamily: "monospace", flex: 1, minWidth: 200, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{embedSnippetFor(auditId)}</code>
                  <button onClick={copySnippet} style={{ padding: "4px 10px", borderRadius: 6, border: "none", cursor: "pointer", background: copiedSnippet ? "linear-gradient(135deg,#186132,#14D571)" : "rgba(0,0,0,0.06)", color: copiedSnippet ? "#fff" : C.navy, fontSize: 10.5, fontWeight: 700 }}>{copiedSnippet ? "Copied" : "Copy"}</button>
                </div>
              )}
              {isLive && activeVariants.length > 0 && (
                <div style={{ padding: "10px 16px", background: "rgba(91,97,244,0.05)", borderBottom: `1px solid ${C.border}` }}>
                  <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: C.violet, marginBottom: 6 }}>Vision Pro live test{activeVariants.length > 1 ? ` — ${activeVariants.length} variants` : ""}</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    {activeVariants.map((v, i) => {
                      ...
                      return <div key={v.id} style={{ fontSize: 11.5, color: "#374151" }}><span style={{ fontWeight: 660 }}>Variant {i + 1}</span> — {weightPct}% traffic · {counts.serves} served · {counts.converts} converted</div>;
                    })}
                  </div>
                </div>
              )}
              {isLive && driftEvents.length > 0 && (
                <div style={{ padding: "10px 16px", background: "rgba(91,97,244,0.05)", borderBottom: `1px solid ${C.border}` }}>
                  <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: C.violet, marginBottom: 6 }}>Drift detected since deploy</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                    {driftEvents.slice(0, 5).map((ev) => (
                      <div key={ev.id} style={{ fontSize: 11.5, color: "#374151" }}>
                        <span style={{ fontWeight: 660 }}>{ev.element || "A section"}</span>{ev.regression_type === "repeated" ? " has repeatedly gotten worse" : " got worse"} on {new Date(ev.detected_at).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}
                        {ev.reasoning && <div style={{ color: C.muted, marginTop: 2 }}>{ev.reasoning}</div>}
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {/* ── UX Pulse ─────────────────────────────────────────────────── */}
      <div id="pulse" style={{ maxWidth: 1120, margin: "0 auto", padding: "44px 28px 24px" }}>
        <h1 style={{ fontFamily: "'Unbounded',sans-serif", fontSize: 26, fontWeight: 700, color: C.navy, letterSpacing: "-0.5px", margin: "0 0 4px" }}>
      ...
      {/* ── Premium teaser: Design Tool Plugins ─────────────────────── */}
      <div style={{ maxWidth: 1120, margin: "0 auto", padding: "0 28px 60px" }}>
        <div style={{ borderRadius: 16, padding: "20px 24px", background: "#EDEDFA", border: "1px solid rgba(91,97,244,0.12)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              <span style={{ fontFamily: "'Unbounded',sans-serif", fontSize: 13, fontWeight: 700, color: C.navy }}>Design Tool Plugins</span>
              <span style={{ fontSize: 9, fontWeight: 700, background: "rgba(91,97,244,0.1)", color: C.violet, borderRadius: 20, padding: "2px 9px" }}>Premium</span>
            </div>
            <div style={{ fontSize: 12.5, color: C.muted, lineHeight: 1.5 }}>Surface this diagnosis contextually in Figma, Framer, Webflow, and WordPress — without leaving your tool.</div>
          </div>
        </div>
      </div>

      <div style={{ textAlign: "center", padding: "0 0 40px" }}>
        <p style={{ fontSize: 11, color: C.dim, margin: 0 }}>{realDomain} · Audit #{auditId} · UXpact © 2026</p>
      </div>
    </div>
  );
}
