...
export default function UXpactEngine() {
  const [name, setName] = useState("");
  const [email, setEmail] = useState("");
  const [url, setUrl] = useState("");
  const [pageGoals, setPageGoals] = useState([]);
  const [industry, setIndustry] = useState("");
  const [challenge, setChallenge] = useState(2);
  const [reviewFocus, setReviewFocus] = useState([]);
  const [hoveredBtn, setHoveredBtn] = useState(false);

  const toggleGoal = (g) => setPageGoals((p) => p.includes(g) ? p.filter((x) => x !== g) : [...p, g]);
  const toggleFocus = (f) => setReviewFocus((p) => p.includes(f) ? p.filter((x) => x !== f) : [...p, f]);
    ...
    <div style={{ minHeight: "100vh", background: "#EEF1F5", fontFamily: "'Space Grotesk', sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: 1060, margin: "0 auto", padding: "8px 28px 60px", position: "relative", zIndex: 10 }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <p style={{ fontSize: 14, color: "#6B7280", margin: 0, fontWeight: 400 }}>Fill in the details below and we'll analyze your site instantly.</p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 290px", gap: 16, alignItems: "stretch" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                ...
                <div><label style={lbl}>What type of site is this? *</label>
                  <select value={industry} onChange={(e) => setIndustry(e.target.value)}
                    style={{ ...inp, appearance: "none", backgroundImage: "url(\"data:image/svg+xml,%3Csvg width='10' height='6' viewBox='0 0 10 6' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1l4 4 4-4' stroke='%236B7280' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E\")", backgroundRepeat: "no-repeat", backgroundPosition: "right 12px center", paddingRight: 28, color: industry ? "#0B1C48" : "#9CA3AF" }}
            ...
            <div style={{ ...glass, padding: "24px", flex: 1 }}>
              ...
              {/* Page goals — colored tags inside input container */}
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8, padding: "10px 14px", borderRadius: 10, background: "rgba(245,246,248,0.8)", border: "1px solid rgba(0,0,0,0.04)", boxShadow: "inset 0 2px 4px rgba(0,0,0,0.04), 0 1px 0 rgba(255,255,255,0.7)", minHeight: 46 }}>
                  {PAGE_GOALS.map((goal) => {
                    const sel = pageGoals.includes(goal);
                    return (
                      <div key={goal} onClick={() => toggleGoal(goal)} style={{
                        padding: "5px 14px", borderRadius: 6,
                        background: sel ? (["Lead gen","Bookings","Brand awareness"].includes(goal) ? "#D1FAE5" : "#E0E7FF") : "rgba(255,255,255,0.85)",
                        border: "none",
                        cursor: "pointer", transition: "all 0.15s",
                        fontSize: 12.5, fontWeight: sel ? 600 : 450,
                        color: sel ? "#0B1C48" : "#6B7280",
                        whiteSpace: "nowrap", boxShadow: sel ? "0 1px 3px rgba(0,0,0,0.06)" : "0 1px 2px rgba(0,0,0,0.03)",
                      }}>{goal}</div>
                    );
                  })}
                </div>
              ...
              <div style={{ marginBottom: 22 }}>
                ...
                <div style={{ padding: "0 2px" }}>
                  <input type="range" min="0" max="4" step="1" value={challenge}
                    onChange={(e) => setChallenge(parseInt(e.target.value))}
                    className="uxpact-slider"
                    style={{ width: "100%", height: 6, outline: "none", cursor: "pointer", margin: 0 }}
                  />
                  <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8 }}>
                    {urg.map((l, i) => (
                      <span key={i} style={{ fontSize: 10.5, color: challenge === i ? "#0B1C48" : "#9CA3AF", fontWeight: challenge === i ? 600 : 400, textAlign: "center", flex: 1, transition: "all 0.2s" }}>{l}</span>
                    ))}
                  </div>
                </div>
              </div>

              {/* Review focus checkboxes */}
              <div style={{ marginBottom: 22 }}>
                <label style={{ ...lbl, marginBottom: 10 }}>What would you like reviewed?</label>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                  {REVIEW_FOCUS.map((item) => {
                    const ch = reviewFocus.includes(item);
                    return (
                      <div key={item} onClick={() => toggleFocus(item)} style={{
                        ...
                        <div style={{
                          ...
                          border: ch ? "none" : "1.5px solid #BFC5CE",
                          background: ch ? "linear-gradient(135deg, #16a34a, #22c55e)" : "#fff",
              ...
              {/* CTA */}
              <button disabled={!isReady} onMouseEnter={() => setHoveredBtn(true)} onMouseLeave={() => setHoveredBtn(false)}
                style={{
                  width: "100%", height: 48, borderRadius: 10, border: "none",
                  background: isReady ? (hoveredBtn ? "linear-gradient(135deg, #126B2E, #148C59)" : "linear-gradient(135deg, #186132, #14D571)") : "rgba(0,0,0,0.06)",
                  color: isReady ? "#fff" : "#9CA3AF",
                  fontFamily: "'Unbounded', sans-serif", fontSize: 14, fontWeight: 660,
                  cursor: isReady ? "pointer" : "not-allowed", transition: "all 0.3s",
                  boxShadow: isReady ? (hoveredBtn ? "0 8px 28px rgba(20,140,89,0.3)" : "0 3px 12px rgba(20,140,89,0.15)") : "none",
                  transform: isReady && hoveredBtn ? "translateY(-1px)" : "none",
              ...
              <div style={{ textAlign: "center", fontSize: 11, color: "#9CA3AF", marginTop: 10, fontWeight: 400 }}>
                Free during beta &middot; Full reports from &pound;X/audit after launch
              </div>
            </div>
          </div>

          {/* RIGHT: Your Audit Pack */}
          <div style={{ ...glass, padding: "22px 18px", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, flex: 1 }}>
              {[
                ...
                { label: "Track your fixes", title: "Pulse Extension", desc: "Chrome & Edge extension to check off fixes as you go", bg: "linear-gradient(135deg, rgba(91,97,244,0.12), rgba(91,97,244,0.05))", color: "#5B61F4", icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M22 11.08V12a10 10 0 11-5.93-9.14" stroke="#5B61F4" strokeWidth="1.5" strokeLinecap="round" /><path d="M22 4L12 14.01l-3-3" stroke="#5B61F4" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg> },
                { label: "Delivery", title: "Instant PDF Download", desc: "Full findings, fixes, and research-backed citations", bg: "linear-gradient(135deg, rgba(20,140,89,0.10), rgba(20,213,113,0.05))", color: "#148C59", icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" stroke="#148C59" strokeWidth="1.5" strokeLinecap="round" /><path d="M14 2v6h6M12 18v-6M9 15l3 3 3-3" stroke="#148C59" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg> },
              ].map((c, i) => (
                <div key={i} style={{ padding: "18px 16px", borderRadius: 10, background: c.bg, border: "1px solid rgba(255,255,255,0.5)", boxShadow: "0 2px 8px rgba(0,0,0,0.03), inset 0 1px 0 rgba(255,255,255,0.5)", flex: 1, display: "flex", alignItems: "flex-start", gap: 12 }}>
                  <div style={{ width: 38, height: 38, borderRadius: 10, background: "rgba(255,255,255,0.7)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, boxShadow: "0 2px 6px rgba(0,0,0,0.04), inset 0 1px 0 rgba(255,255,255,0.8)" }}>{c.icon}</div>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 10, fontWeight: 600, color: c.color, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 3 }}>{c.label}</div>
                    <div style={{ fontFamily: "'Unbounded', sans-serif", fontSize: 12, fontWeight: 660, color: "#0B1C48", marginBottom: 4, lineHeight: 1.3 }}>{c.title}</div>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; }
        ...
        .uxpact-slider::-moz-range-track { height: 6px; border-radius: 3px; background: linear-gradient(90deg, #14D571 0%, #148C59 50%, #5B61F4 100%); }
      `}</style>
    </div>
  );
}
