...
// ── Main ──────────────────────────────────────────────────────────────
export default function ConversionBlueprint() {
  const [activeId, setActiveId] = useState(null);
  const [facView, setFacView] = useState("current");
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Space Grotesk', sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 10 }}>
        ...
        <div style={{ maxWidth: 1120, margin: "0 auto", padding: "0 28px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
          ...
          {/* Right side — severity counts + view toggle */}
          <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: facView === v
                      ? v === "restructured"
                        ? "linear-gradient(135deg, #186132, #148C59)"
                        : "#fff"
                      : "transparent",
                    color: facView === v
                      ? v === "restructured" ? "#fff" : C.navy
                      : C.muted,
        ...
        <div style={{ maxWidth: 1120, margin: "0 auto", padding: "0 28px 12px", display: "flex", justifyContent: "flex-end" }}>
          <span style={{ fontSize: 11, color: C.dim, fontFamily: "'Space Grotesk', sans-serif" }}>
            {facView === "current" ? "Click a pin to see the fix + AI prompt" : "Sections restructured for Hero archetype — 2 rewritten · 1 added · 1 moved"}
          </span>
        </div>
        ...
        <div style={{ maxWidth: 1120, margin: "0 auto", padding: "0 28px 0", display: "flex", gap: 16, alignItems: "flex-start" }}>
          ...
          <div style={{
            ...
            <FacSection style={{ padding: "13px 28px", background: "rgba(255,255,255,0.35)" }}>
              ...
              {facView === "current" && <PinRow zone="nav" {...pinProps} />}
            </FacSection>
            ...
            {facView === "current" ? (
              ...
              <FacSection style={{ textAlign: "center", padding: "44px 48px 36px", background: "linear-gradient(180deg, rgba(209,250,229,0.1) 0%, transparent 100%)", position: "relative" }}>
                <div style={{ position: "absolute", top: 12, right: 14, fontSize: 9, fontWeight: 600, padding: "2px 8px", borderRadius: 4, background: "#EEF2FF", color: "#3730A3", fontFamily: "'Space Grotesk', sans-serif" }}>Rewritten</div>
                <div style={{ fontSize: 27, fontWeight: 700, color: C.navy, fontFamily: "'Unbounded', sans-serif", letterSpacing: "-0.5px", lineHeight: 1.25, marginBottom: 12 }}>
                  Know exactly where you're losing customers
                </div>
                <div style={{ fontSize: 13.5, color: C.muted, fontFamily: "'Space Grotesk', sans-serif", marginBottom: 22, maxWidth: 420, margin: "0 auto 22px" }}>
                  See every friction point in your funnel — before they cost you another conversion.
                </div>
                <div style={{ display: "flex", justifyContent: "center", gap: 10 }}>
                  <div style={{ padding: "9px 26px", background: "linear-gradient(135deg, #186132, #148C59)", borderRadius: 8, color: "#fff", fontSize: 13, fontWeight: 600, fontFamily: "'Space Grotesk', sans-serif" }}>
                    Start free — no credit card
                  </div>
                  <div style={{ padding: "9px 20px", background: "transparent", border: `1px solid ${C.border}`, borderRadius: 8, color: C.navy, fontSize: 13, fontWeight: 500, fontFamily: "'Space Grotesk', sans-serif" }}>
                    See a 2-min demo
                  </div>
                </div>
              </FacSection>
            ...
            {/* PROOF BAR — restructured only, appears before features */}
            {facView === "restructured" && (
              <FacSection style={{ background: "rgba(209,250,229,0.12)", position: "relative" }}>
                <div style={{ position: "absolute", top: 12, right: 14, fontSize: 9, fontWeight: 600, padding: "2px 8px", borderRadius: 4, background: "#F0FDF4", color: "#065F46", fontFamily: "'Space Grotesk', sans-serif" }}>Added</div>
                <div style={{ display: "flex", justifyContent: "space-around", textAlign: "center" }}>
                  {[["1,200+","teams synced"],["99.9%","sync accuracy"],["8 min","avg. setup"]].map(([n, l], i) => (
                    <div key={i}>
                      <div style={{ fontSize: 20, fontWeight: 700, color: C.forest, fontFamily: "'Unbounded', sans-serif" }}>{n}</div>
                      <div style={{ fontSize: 11, color: C.muted, fontFamily: "'Space Grotesk', sans-serif", marginTop: 3 }}>{l}</div>
                    </div>
                  ))}
                </div>
              </FacSection>
            ...
            {facView === "current" ? (
              ...
              <FacSection style={{ position: "relative" }}>
                <div style={{ position: "absolute", top: 12, right: 14, fontSize: 9, fontWeight: 600, padding: "2px 8px", borderRadius: 4, background: "#FEF3C7", color: "#92400E", fontFamily: "'Space Grotesk', sans-serif" }}>Moved</div>
                <FacLabel t="Features" />
                <FacH2>Built around how you actually work</FacH2>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 11, marginTop: 14 }}>
                  {[
                    { t: "See every drop-off instantly", b: "Real-time dashboards show you exactly where users leave — no digging required." },
                    { t: "Reports your team will actually use", b: "Generate any report in seconds. Your workflow, not ours." },
                    { t: "Your whole team, one view", b: "Everyone stays aligned on what's working and what needs fixing." },
                  ].map((c, i) => (
                    <div key={i} style={{ background: "rgba(255,255,255,0.55)", border: `1px solid ${C.border}`, borderRadius: 9, padding: "13px 14px" }}>
                      <div style={{ fontSize: 12.5, fontWeight: 650, color: C.navy, fontFamily: "'Space Grotesk', sans-serif", marginBottom: 4 }}>{c.t}</div>
                      <div style={{ fontSize: 11.5, color: C.muted, lineHeight: 1.6, fontFamily: "'Space Grotesk', sans-serif" }}>{c.b}</div>
            ...
            {facView === "current" ? (
              ...
              <FacSection style={{ background: "rgba(224,231,255,0.07)", position: "relative" }}>
                <div style={{ position: "absolute", top: 12, right: 14, fontSize: 9, fontWeight: 600, padding: "2px 8px", borderRadius: 4, background: "#EEF2FF", color: "#3730A3", fontFamily: "'Space Grotesk', sans-serif" }}>Rewritten</div>
                <FacLabel t="Customers" />
                <FacH2>Teams that switched never went back</FacH2>
                <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginTop: 14 }}>
                  {[100,80,110,90,70].map((w,i) => (
                    <div key={i} style={{ width: w, height: 26, background: "rgba(0,0,0,0.06)", borderRadius: 5 }} />
                  ))}
                ...
                <div style={{ marginTop: 12, padding: "12px 14px", background: "rgba(209,250,229,0.2)", border: "1px solid rgba(20,140,89,0.15)", borderRadius: 8 }}>
                  <span style={{ fontSize: 12, color: C.navy, fontFamily: "'Space Grotesk', sans-serif", fontStyle: "italic", lineHeight: 1.6 }}>"We cut reconciliation time from 4 hours to 15 minutes in the first week." — Sarah M., Head of Finance</span>
                </div>
              </FacSection>
            )}
            ...
            {facView === "current" ? (
              ...
              <FacSection style={{ textAlign: "center", background: "linear-gradient(135deg, #186132, #0B1C48)", position: "relative" }}>
                <div style={{ position: "absolute", top: 12, right: 14, fontSize: 9, fontWeight: 600, padding: "2px 8px", borderRadius: 4, background: "#EEF2FF", color: "#3730A3", fontFamily: "'Space Grotesk', sans-serif" }}>Rewritten</div>
                <div style={{ fontSize: 17, fontWeight: 700, color: "#fff", fontFamily: "'Unbounded', sans-serif", letterSpacing: "-0.3px", marginBottom: 8 }}>Ready to get your time back?</div>
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.7)", fontFamily: "'Space Grotesk', sans-serif", marginBottom: 20 }}>Join 1,200+ teams already saving hours every week. Setup takes 8 minutes.</div>
                <div style={{ display: "flex", justifyContent: "center", gap: 10 }}>
                  <div style={{ padding: "9px 26px", background: C.mint, borderRadius: 8, color: C.navy, fontSize: 13, fontWeight: 700, fontFamily: "'Space Grotesk', sans-serif" }}>
                    Start free — no credit card
                  </div>
                </div>
              </FacSection>
        ...
        <div style={{ maxWidth: 1120, margin: "0 auto", padding: "8px 28px 24px", textAlign: "center" }}>
          <span style={{ fontSize: 11, color: C.dim, fontFamily: "'Space Grotesk', sans-serif", letterSpacing: "0.06em" }}>Audit #0024</span>
        </div>
