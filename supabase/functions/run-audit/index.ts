...
Deno.serve(async (req: Request): Promise<Response> => {
    ...
    return jsonResponse(
      {
        ...
        status: "failed_spa",
        error:
          "Page appears to be a JavaScript-rendered SPA with insufficient server-rendered content.",
      },
      422,
    );
  }
  ...
  if (!doc) {
    return jsonResponse(
      { success: false, status: "failed_error", error: "Failed to parse HTML" },
      500,
    );
  }
  ...
  return jsonResponse({ success: true, metadata }, 200);
});
...
async function fetchHtml(
  url: string,
): Promise<{ success: true; html: string; response: Response } | {
  success: false;
  error: string;
}> {
...
function extractPageMetadata(args: {
  doc: Document;
  html: string;
  url: URL;
  statusCode: number;
  ...
  const headingHierarchy = Array.from(doc.querySelectorAll("h1, h2, h3, h4, h5, h6")).map(
    (heading) => {
  ...
  const navLinks = Array.from(doc.querySelectorAll("nav a[href]")).map((anchor) => ({
  ...
  const allLinks = Array.from(doc.querySelectorAll("a[href]")).map((anchor) => {
    ...
    return {
      text: cleanText(anchor.textContent),
      href,
      isExternal: resolved ? resolved.origin !== url.origin : false,
    };
  });
  ...
  const buttonTexts = Array.from(doc.querySelectorAll("button"))
  ...
  return {
    ...
    charset: doc.querySelector("meta[charset]")?.getAttribute("charset") ??
      extractCharsetFromContentType(doc.querySelector('meta[http-equiv="Content-Type" i]')?.getAttribute("content")),
    jsonLdBlocks,
