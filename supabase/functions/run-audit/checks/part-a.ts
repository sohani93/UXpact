...
export function runPartAChecks(metadata: PageMetadata): CheckResult[] {
  ...
  if (metadata.h1Count < 1 || !h1Text) {
    checks.push(result({ id: "A1.1", name: "Hero headline missing", part: "A", category: "First Impression & Clarity", severity: "critical", pass: false, score: 0, finding: "No hero headline (H1) found. Visitors can't tell what this site is about.", fix: "Add a clear H1 headline that communicates your value proposition in under 10 words." }));
  } else if (h1Words > 10) {
    checks.push(result({ id: "A1.1", name: "Hero headline too long", part: "A", category: "First Impression & Clarity", severity: "critical", pass: false, score: 4, finding: `H1 exists but is too long (${h1Words} words). Keep it under 10.`, fix: "Shorten your H1 to a concise value proposition with fewer than 10 words." }));
  } else {
  ...
  checks.push(result({
    id: "A1.2",
    name: hasSupportingH2 || hasSupportingParagraph ? "Supporting subheadline exists" : "Supporting subheadline missing",
    part: "A",
  ...
  if (metadata.ctaCount < 1) {
    checks.push(result({ id: "A1.4", name: "Primary CTA missing", part: "A", category: "First Impression & Clarity", severity: "critical", pass: false, score: 0, finding: "No call-to-action button found. Users have nowhere to go.", fix: "Add a prominent CTA button in the hero section with an action verb (e.g., 'Start free trial', 'Get your audit')." }));
  } else if (GENERIC_CTA_TEXTS.has(firstCtaText)) {
    checks.push(result({ id: "A1.4", name: "Primary CTA copy is weak", part: "A", category: "First Impression & Clarity", severity: "critical", pass: false, score: 4, finding: `CTA exists but uses weak text: '${ctaTexts[0]}'`, fix: "Rewrite CTA to use verb + benefit format. E.g., 'Get my free report' instead of 'Submit'." }));
  } else {
  ...
  checks.push(result({
    id: "A1.5", name: metadata.ctaCount >= 2 ? "Secondary CTA exists" : "Secondary CTA missing", part: "A", category: "First Impression & Clarity", severity: "minor",
  ...
  checks.push(result({
    id: "A1.8", name: aboveFoldElementEstimate <= 9 ? "Above-fold not cluttered" : "Above-fold clutter detected", part: "A", category: "First Impression & Clarity", severity: "major",
  ...
  if (metadata.navLinkCount < 1) {
    checks.push(result({ id: "A2.1", name: "Navigation missing", part: "A", category: "Navigation & Structure", severity: "critical", pass: false, score: 0, finding: "No navigation menu found.", fix: "Add a navigation bar with 5-7 clearly labeled menu items." }));
  } else if (metadata.navLinkCount > 7) {
    checks.push(result({ id: "A2.1", name: "Navigation overloaded", part: "A", category: "Navigation & Structure", severity: "critical", pass: false, score: 4, finding: `Navigation has ${metadata.navLinkCount} items. Hick's Law recommends 5-7 max.`, fix: "Reduce primary navigation to the most important 5-7 items." }));
  } else {
  ...
  checks.push(result({
    id: "A2.3", name: metadata.logoLinksHome ? "Logo links to homepage" : "Logo may not link to homepage", part: "A", category: "Navigation & Structure", severity: "minor",
  ...
  checks.push(result({
    id: "A2.5", name: metadata.hasViewportMeta ? "Mobile nav is accessible" : "Mobile nav accessibility risk", part: "A", category: "Navigation & Structure", severity: "critical",
  ...
  checks.push(result({
    id: "A3.2", name: longParagraphShare <= 0.5 ? "Body copy is scannable" : "Body copy not scannable", part: "A", category: "Copy & Messaging", severity: "major",
  ...
  checks.push(result({
    id: "A3.4", name: !genericButton ? "Microcopy on forms/buttons is clear" : "Microcopy clarity issue", part: "A", category: "Copy & Messaging", severity: "major",
  ...
  checks.push(result({
    id: "A3.5", name: !placeholderMatch ? "No orphaned or placeholder text" : "Placeholder text detected", part: "A", category: "Copy & Messaging", severity: "critical",
  ...
  checks.push(result({ id: "A4.1", name: hasSocialSignals ? "Social proof exists" : "Social proof missing", part: "A", category: "Trust & Social Proof", severity: "critical", pass: hasSocialSignals, score: hasSocialSignals ? 10 : 0, finding: hasSocialSignals ? "Social proof signals were detected." : "No social proof found (testimonials, reviews, client logos, or trust badges).", fix: "Add at least 2-3 testimonials with full name + role + specific result. Or add a client logo bar." }));
  ...
  checks.push(result({ id: "A4.5", name: a45Pass ? "Security signals for transactional sites" : "Security signals missing", part: "A", category: "Trust & Social Proof", severity: "critical", pass: a45Pass, score: a45Pass ? 10 : 0, finding: a45Pass ? "HTTPS and relevant security/privacy signals are present." : "Site collects data via forms but shows no security or privacy signals.", fix: "Add a privacy policy link near forms. Display SSL/security badges if processing payments." }));
  ...
  checks.push(result({ id: "A5.3", name: a53Score >= 7 ? "CTA copy is action-oriented" : "CTA copy lacks action/benefit", part: "A", category: "CTA & Conversion Design", severity: "major", pass: a53Score >= 7, score: a53Score, finding: a53Finding, fix: "Rewrite to: [verb] + [benefit]. Example: 'Get my free audit' instead of 'Submit'." }));
  const uniqueCtas = new Set(ctaTexts.map((text) => text.toLowerCase()));
  checks.push(result({ id: "A5.4", name: uniqueCtas.size <= 3 ? "No competing CTAs" : "Competing CTAs detected", part: "A", category: "CTA & Conversion Design", severity: "major", pass: uniqueCtas.size <= 3, score: uniqueCtas.size <= 3 ? 10 : 4, finding: uniqueCtas.size <= 3 ? "CTAs are focused with clear primary action." : `Found ${uniqueCtas.size} different CTA labels competing for attention. Hick's Law: too many choices = no choice.`, fix: "Consolidate to 1 primary CTA (repeated) + 1 secondary action max per section." }));
  ...
  checks.push(result({ id: "A6.2", name: "Typography readability issue", part: "A", category: "Layout & Visual Design", severity: "critical", pass: false, score: 0, manualReview: true, finding: "Typography readability requires visual inspection. Ensure body text is ≥16px with ≥1.5 line height.", fix: "Review rendered text size and spacing on desktop and mobile." }));
  ...
  checks.push(result({ id: "A7.2", name: a72Score === 10 ? "Heading hierarchy is correct" : "Heading hierarchy issue", part: "A", category: "Technical Readiness — ShipCheck", severity: "major", pass: a72Score === 10, score: a72Score, finding: a72Finding, fix: "Use exactly one H1, then H2 for sections, H3 for subsections. Don't skip levels." }));
