...
export function runPartBChecks(metadata: PageMetadata, industry: Industry): CheckResult[] {
  ...
  if (industry === "ecommerce") {
    ...
    return [
      ...
      buildResult({ id: "B2.2", name: "Price is visible", part: "B", category: "Industry-Specific (Ecommerce)", severity: "critical", pass: priceVisible, score: priceVisible ? 10 : 0, manualReview: false, finding: priceVisible ? "Price signals are visible." : "No visible pricing. 47% of users abandon due to unexpected costs (Baymard).", fix: "Make the price immediately visible without scrolling." }),
  ...
  if (industry === "portfolio") {
    ...
    return [
      ...
      buildResult({ id: "B3.2", name: "About/bio is specific", part: "B", category: "Industry-Specific (Portfolio)", severity: "major", pass: about, score: about ? 10 : 0, manualReview: false, finding: about ? "About/bio signals detected." : "No clear about/bio section with personal details.", fix: "Add an about section with your name, photo, specific expertise, and years of experience." }),
      buildResult({ id: "B3.3", name: "Contact method is obvious", part: "B", category: "Industry-Specific (Portfolio)", severity: "critical", pass: contact, score: contact ? 10 : 0, manualReview: false, finding: contact ? "A contact path is visible." : "No clear way to contact you found.", fix: "Add a visible contact form, email link, or scheduling link (Calendly etc.) accessible from any page." }),
  ...
  if (industry === "healthcare") {
    ...
    return [
      buildResult({ id: "B4.1", name: "Medical disclaimers", part: "B", category: "Industry-Specific (Healthcare)", severity: "critical", pass: disclaimer, score: disclaimer ? 10 : 0, manualReview: false, finding: disclaimer ? "Medical disclaimer language found." : "No medical disclaimers found. Required for any site making health claims.", fix: "Add visible medical disclaimers. Don't bury them in the footer only." }),
      manual("B4.2", "Clinical evidence quality", "Industry-Specific (Healthcare)", "major"),
      buildResult({ id: "B4.3", name: "Privacy and data handling", part: "B", category: "Industry-Specific (Healthcare)", severity: "critical", pass: privacy, score: privacy ? 10 : 0, manualReview: false, finding: privacy ? "Privacy/data handling signals found." : "No privacy or data handling signals. Users sharing health data need reassurance.", fix: "Add explicit privacy/data handling statements. Link to your privacy policy prominently." }),
  ...
  if (industry === "fintech") {
    ...
    return [
      buildResult({ id: "B5.1", name: "Regulatory badges", part: "B", category: "Industry-Specific (Fintech)", severity: "critical", pass: regulatory, score: regulatory ? 10 : 0, manualReview: false, finding: regulatory ? "Regulatory/compliance language detected." : "No regulatory or compliance badges found. Critical for financial trust.", fix: "Display FCA/SEC registration, licensing numbers, or regulatory compliance badges." }),
      manual("B5.2", "Risk disclosure clarity", "Industry-Specific (Fintech)", "critical"),
      buildResult({ id: "B5.3", name: "Fee transparency", part: "B", category: "Industry-Specific (Fintech)", severity: "major", pass: fee, score: fee ? 10 : 0, manualReview: false, finding: fee ? "Fee transparency signals detected." : "No fee information visible. Hidden fees instantly destroy trust in finance.", fix: "Show all fees clearly. Add 'No hidden fees' badge if applicable." }),
  ...
  if (industry === "service") {
    ...
    return [
      buildResult({ id: "B6.1", name: "Services are clearly listed", part: "B", category: "Industry-Specific (Service/Agency)", severity: "critical", pass: services, score: services ? 10 : 0, manualReview: false, finding: services ? "Service/offering language is present." : "No clear service listing. 'We help businesses grow' is not a service description.", fix: "List your specific services with 1-line descriptions each. Be concrete." }),
      buildResult({ id: "B6.2", name: "Process shown", part: "B", category: "Industry-Specific (Service/Agency)", severity: "major", pass: process, score: process ? 10 : 0, manualReview: false, finding: process ? "Process/methodology section found." : "No 'how we work' section. Users need to know what happens after they contact you.", fix: "Add a process section: Discovery → Strategy → Execution → Results (or your version)." }),
      ...
      buildResult({ id: "B6.5", name: "Booking is frictionless", part: "B", category: "Industry-Specific (Service/Agency)", severity: "major", pass: bookingPass, score: bookingPass ? 10 : 0, manualReview: false, finding: bookingPass ? "Low-friction booking/contact method detected." : `No easy booking option or the inquiry form is too long (${metadata.inputFields.length} fields).`, fix: "Add a Calendly/Cal.com embed or short contact form (name + email + message max)." }),
    ];
  }
